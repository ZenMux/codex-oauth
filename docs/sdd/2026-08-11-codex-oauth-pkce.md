...
The production installer queries `/api/frontend/model/listByFilter?supported_protocol=responses`, defensively keeps only entries whose `suitable_api` contains the exact `responses` protocol, writes a private Codex-schema catalog under the OAuth state directory, and configures its path through root-level `model_catalog_json`. OpenAI GPT-series entries are stably ordered before other families because they support Codex's Responses cache controls; order within the GPT and non-GPT groups remains unchanged. GPT entries use their first production native alias (for example `gpt-5.6-sol`) to preserve Codex model-specific behavior; other providers retain their full ZenMux slug. The catalog disables first-party-only Responses Lite and hosted tool mode. OpenAI GPT entries set `multi_agent_version` to `v2`, while config sets `[features.multi_agent_v2] tool_namespace = "agents"`, so Codex 0.144.1 and newer avoid conflicts with model-reserved `collaboration` tool names. Other model families leave the explicit version unset (`null`) and follow Codex's default multi-agent behavior.
